...
#################################################
#################################################
# 1D with math symbols
function load_math1Dlabels(vars)
    l = Dict()
    # Uppercase
...
# 1D without math symbols:
function load_text1Dlabels(vars)
    l = Dict()
    # Uppercase
    l["A_ice"] = L"Ice surface coverage ($10^6$ $\mathrm{km}^2$)"
    # l["A_ice_f"] = L"$A_{\mathrm{ice,f}}$ [$10^6$ $\mathrm{km}^2$]"
    # l["A_ice_g"] = L"$A_{\mathrm{ice,g}}$ [$10^6$ $\mathrm{km}^2$]"
    l["H_ice"] = L"Ice thickness (m) $\,$"
    # l["H_ice_f"] = L"$H_{\mathrm{ice,f}}$ [m]"
    # l["H_ice_g"] = L"$A_{\mathrm{ice,g}}$ [m]"
    # l["H_ice_max"] = L"$H_{\mathrm{ice, max}}$ [m]"
    # l["H_w"] = L"$H_{w}$ [m]"
    ...
    # l["T_srf"] = L"$T_{srf}$ [K]"
    # l["V_dT"] = L"$V_{T}$ []"
    l["V_ice"] = L"Ice volume ($10^6$ $\mathrm{km}^3$)"
    # l["V_ice_f"] = L"$V_{\mathrm{ice,f}}$ [$10^6$ $\mathrm{km}^3$]"
    # l["V_ice_g"] = L"$V_{\mathrm{ice,g}}$ [$10^6$ $\mathrm{km}^3$]"
    l["V_sle"] = L"Sea-level equivalent ice volume (mSLE) $\,$"
    ...
    l["dHicedt"] = L"Ice thickness rate (m$ \, \mathrm{yr}^{-1}$)"
    l["dVicedt"] = L"Ice volume rate  ($10^{-3}$ [$\mathrm{km}^3 \, \mathrm{yr}^{-1}$)"
    # l["dzsrfdt"] = L"$\frac{d z_{srf}}{d t}$ [m$ \, \mathrm{yr}^{-1}$]"
    # l["f_pmp"] = L"$f_{pmp}$ [1]"
    # l["fwf"] = L"$fwf$ [Sv]"
    # l["hyst_df_dt"] = L"$\frac{d m}{d t}_{base}$ [1]"
    ...
    # l["uxy_bar_f"] = L"$\bar{u}_{xy,f}$ [m$ \, \mathrm{yr}^{-1}$]"
    # l["uxy_bar_g"] = L"$\bar{u}_{xy,g}$ [m$ \, \mathrm{yr}^{-1}$]"
    l["uxy_s"] = L"Ice surface velocity (m$ \, \mathrm{yr}^{-1}$)"
    # l["uxy_s_f"] = L"$u_{xy,s,f}$ [m$ \, \mathrm{yr}^{-1}$]"
    # l["uxy_s_g"] = L"$u_{xy,s,g}$ [m$ \, \mathrm{yr}^{-1}$]"
    # l["xc"] = L"$\frac{d m}{d t}_{base}$ [1]"
    # l["yc"] = L"$\frac{d m}{d t}_{base}$ [1]"
    l["z_bed"] = L"Bedrock elevation (m) $\,$"
    # l["z_sl"] = L"$z_{sl}$ [m]"
    l["z_srf"] = L"Surface elevation (m) $\,$"
    # l["z_srf_g"] = L"$z_{srf,g}$ [m]"
    # l["zeta"] = L"$\frac{d m}{d t}_{base}$ [1]"
...
# 3D with math symbols:
function load_math3Dlabels(vars)
    l = Dict()
    l["H_ice"] = L"$H_{\mathrm{ice}}$ [m]"
    ...
    l["taub"] = L"Basal shear stress [Pa]" # $\tau_\mathrm{b}$
    l["uxy_s"] = L"$u_{xy,s}$ [m$ \, \mathrm{yr}^{-1}$]"
...
# 3D without math symbols:
function load_text3Dlabels(vars)
    l = Dict()
    l["H_ice"] = L"Ice thickness (m)"
    l["H_ice_pd_err"] = L"Ice thickness deviation (m)"
    # l["N_eff"] = L"$N_{eff}$ [bar]"
    # l["T_srf"] = L"Surface temperature (K)"
    # l["Ta_ann"] = L"$\Delta T_{a}$ [K]"

    l["bmb"] = L"Basal mass balance (m$ \, \mathrm{yr}^{-1}$)"
    l["calv"] = L"Calving rate (m$ \, \mathrm{yr}^{-1}$)"
    l["f_grnd"] = L"Fraction of grounded ice (1)"
    l["f_ice"] = L"$Fraction of ice (1)"
    l["mb_applied"] = L"Total mass balance (m$ \, \mathrm{yr}^{-1}$)"
    l["smb"] = L"Surface mass balance (m$ \, \mathrm{yr}^{-1}$)"
    l["taub"] = L"Basal shear stress (Pa)"
    l["uxy_s"] = L"Ice surface velocity (m$ \, \mathrm{yr}^{-1}$)"
    l["uxy_s_pd_err"] = L"Ice surface velocity deviation (m$ \, \mathrm{yr}^{-1}$)"
    l["visc_eff_int"] = L"Effective viscosity (Pa $\cdot$ yr $\cdot$ m)"
    l["z_bed"] = L"Bedrock elevation (m)"
    l["z_srf"] = L"Surface elevation (m)"
    l["z_srf_pd_err"] = L"Surface elevation deviation (m)"
